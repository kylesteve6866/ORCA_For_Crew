...
//构建场景
/*
    std::vector<float >> obstacles：记录障碍物信息
    std::vector<float>agent_ID：记录智能体 ID 信息
    std::vector<std::vector<float>> agent_Pos：记录智能体位置信息
    std::vector<std::vector<float>> agent_Goals：记录智能体目标信息
*/
void setupScenario(RVO::RVOSimulator* sim, std::vector<std::vector<float>> obstacles, std::vector<float>agent_ID, std::vector<std::vector<float>> agent_Pos, std::vector<std::vector<float>> agent_Goals)
{
#if RVO_SEED_RANDOM_NUMBER_GENERATOR
    ...
    //！！！以下代码为V0.0中代码，由于V0.0对所有的智能体视为同一大小，因而使用setAgentDefaults设置了所有智能体的默认属性。但在V0.1中，我们对不同的智能体指定了不同的信息，因而下面的代码作废
    ...
    */
    ...
    for (size_t i = 0; i < elementCount; ++i) {
        ...
        考虑到环境中会有至高半径为1170的智能体，我们需要将这个值设置为至少远大于这个半径的二倍。此处设置为了5000.0f
        */
        sim->addAgent(RVO::Vector2(pos_x, pos_y), 5000.0f, 20, 5.0f, 5.0f, radius, 2.0f, RVO::Vector2(0.0f, 0.0f));
...
int main() {
    ...
    /* Set up the scenario. */
    setupScenario(sim, obstacles, agent_ID, agent_Pos, agent_Goals);
